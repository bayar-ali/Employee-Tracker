...
var updateEmployeeStatus = {
    employeesNewStatus: function () {
        connection.query("SELECT id, fist_name, last_name, FROM employees", (err, employeeList) => {
            if (err) throw (err)
            var i;
            var listAll = []
            for (i = 0; i < employeeList.length; i++) {
                var employee = {
                    name: `${employeeList[i].fist_name} ${employeeList[i].last_name}, ${employeeList[i]} `,
                    value: {
            ...
            inquirer.prompt(
                ...
                .then((roleUpdate) => {
                    var table = "SELECT DISTINCT  department_name FROM employees INNER JOIN roles ON employees = roles.department_name_id INNER JOIN department ON roles.department_name_id = department.id;"
